...
declare function local:jsonifyDynams($dynams) {
    let $strings := for $elem in $dynams
...
declare function local:jsonifyDynamChoices($choicesDynam) {
    let $strings := for $elem in $choicesDynam
...
declare function local:jsonifyDynamChoice($dynams, $choicesDynam) {
        let $test4 := local:jsonifyDynams($dynams)
        let $test3 :=  local:jsonifyDynamChoices($choicesDynam)
...
(:concat('["',string-join($sources/@xml:id,'","'),'"]'):)
  (
    ...
    '],"children":[',
        local:jsonifyDynamChoice($dynams, $choicesDynam), 
