/**
 * Creates class pmdCE.view.tabPanel.buttonDialogs.cards.ChoiceTstampStaffCard that extend from pmdCE.view.tabPanel.buttonDialogs.cards.ChoiceCard.
 * @class
 * @classdesc pmdCE.view.tabPanel.buttonDialogs.cards.ChoiceTstampStaffCard is class for create
 * two cards for 'create'- or 'change to'-dialog. Card 2 of dialog consists
 * in orig-group, 4 regs and verovio view.
 * This dialog is template for create choices depended on tstamp and staff.
 */
Ext.define('pmdCE.view.tabPanel.buttonDialogs.cards.ChoiceTstampStaffCard', {
	...
	initComponent: function () {
		...
		if (Ext.getCmp('cemain').getCard() === 12) {
			if (Ext.getCmp('cemain').getComponentType().indexOf('Hairpin') > -1) {
	...
	handleCreateButton: function () {
		if (placeField.isValid() && formField.isValid() && tstampFieldOrig.isValid() && tstamp2FieldOrig.isValid() && satffFieldBetween.isValid() && tstampFieldReg1.isValid() && tstampFieldReg2.isValid() && tstampFieldReg3.isValid() && tstampFieldReg4.isValid() && tstamp2FieldReg1.isValid() && tstamp2FieldReg2.isValid() && tstamp2FieldReg3.isValid() && tstamp2FieldReg4.isValid() && staffFieldReg3.isValid() && placeFieldReg3.isValid() && staffFieldReg4.isValid() && placeFieldReg4.isValid()) {
